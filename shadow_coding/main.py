...
class Shadow_CodingCLI:
    """
    【Shadow_Coding V2 总控中心】：支持 L2 跨文件影子契约。
    """
    ...
    def resolve_contracts(self):
        ...
        for symbol, module in self.analyzer.imports.items():
            ...
            potential_file = f"shadow_coding/core/{symbol.lower()}.py"
            if not os.path.exists(potential_file):
                 potential_file = f"shadow_coding/core/{module.split('.')[-1]}.py"
...
if __name__ == "__main__":
    # --- 演示 V2 跨文件契约功能 ---
    demo_file = "shadow_coding/main.py"
    print(f"🚀 [Shadow_Coding V2] Starting secure orchestration for: {demo_file}")

    cli = Shadow_CodingCLI(demo_file)
    ...
    # 模拟提取 main.py 的一段代码
    # 查找 Shadow_CodingCLI 类的 ID
    target_node = None
    for n in cli.protocol.spine:
        if n.t == "Shadow_CodingCLI":
            target_node = n
            break
